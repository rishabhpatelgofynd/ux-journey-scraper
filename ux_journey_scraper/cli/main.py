...
from ux_journey_scraper.core.journey_recorder import JourneyRecorder, Journey
...
@click.option('--format', type=click.Choice(['html', 'json', 'both']), default='both', help='Report format')
@click.option('--guidelines', type=click.Choice(['all', 'essential', 'high']), default='all', help='Guideline priority')
def analyze(journey_file, output_dir, format, guidelines):
    ...
    click.echo(f"⚠️  Total Issues: {analysis.total_violations + analysis.total_accessibility_issues}")
